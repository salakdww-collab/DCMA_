...
objective <- tolower(Sys.getenv("DCMA_OBJECTIVE", "wgr"))
if (!identical(objective, "wgr")) {
  stop("This review package includes only DCMA_OBJECTIVE=wgr for the objective-comparison runner.")
}
method_label <- "DCMA-WGR"
out_dir <- file.path(bench_dir, "results", paste0("dcma_", objective))
